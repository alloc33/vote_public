...
pub mod governance {
    ...
    pub fn ensure_user_can_vote(
        ...
        if user_ttt_amount < vote_fee {
            ...
            msg!("User already has {} TTT tokens", user_ttt_amount);
        }
    ...
    /// - `mint`: The token mint associated with the governance token (TTT).
    /// - `token`: The voter's token account holding TTT tokens.
